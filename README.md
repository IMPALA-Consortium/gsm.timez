...
- **`Analyze_TimeZFunnel()`**: Applies
  `gsm.core::Analyze_NormalApprox()` across sequential months, producing
  funnel plot scores for each site over time.
- **`Flag()`**: Extends `gsm.core::Flag()` with sparse-month awareness
  for time-series data.
- **`PredictBounds_TimeZFunnel()`**: Applies
  `gsm.core::Analyze_NormalApprox_PredictBounds()` across sequential
  months to produce funnel bounds for visualization.
- **`Visualize_Funnel()`**: funnel plot for a single selected month,
  showing site Metric values against their Denominator with Normal
  approximation control bounds.
- **`Visualize_Heatmap()`**: heatmap showing each site’s flag status
  over time.
- **`Visualize_Site()`**: line plot highlighting a single site’s
  cumulative metric trajectory against all other sites, marking flagged
  months.
